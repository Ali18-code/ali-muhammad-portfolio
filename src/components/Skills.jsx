        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {categories.map((cat, idx) => {
            ...
            return (
              <div
                key={idx}
                className="p-5 rounded-xl border border-slate-800/90 bg-slate-900/40 flex flex-col justify-between"
              >
                <div>
                    ...
                    {cat.items.map((skill, sIdx) => (
                      <div key={sIdx} className="group">
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span className={`text-[10px] font-mono px-2 py-0.5 rounded border ${cat.badgeColor}`}>
                            {skill.level}
                          </span>
                        </div>
